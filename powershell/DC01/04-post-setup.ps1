...
foreach ($ou in $OUlist) {
    New-Item -Path "{$driveLetter}:\$ou" -ItemType Directory -Force
...
foreach ($ou in $OUlist) {
    for ($i = 1; $i -le $usersPerOU; $i++) {
        ...
        New-ADUser `
            ...
            -Path "OU=$ou,DC=torbenbyg,DC=local" `
            -Enabled $true
    }
}
...
foreach ($ou in $OUlist) {
    ...
    foreach ($user in $users) {
        Add-ADGroupMember -Identity $globalGroup -Members $user.SamAccountName
    }
...
foreach ($ou in $OUlist) {
    ...
    icacls $folderPath /grant "${localGroup}:(OI)(CI)F" | Out-Null
}
